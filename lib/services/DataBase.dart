...
class DataBase {
  ...
  Future<List<RidesModel>> getRidesModelsFromSearch(
      SearchModel searchModel) async {
    String currentUser = await auth.getCurrentFireBaseUserID();

    //Destination set when searching for ride
    GeoFirePoint toGeoFirePoint = GeoFirePoint(
        searchModel.toCords.latitude, searchModel.toCords.longitude);
    ...
    var geoRef = geo.collection(collectionRef: ridesModelCollection).within(
        center: toGeoFirePoint, radius: 3, field: 'toLatLng', strictMode: true);

    var listFromStream = await geoRef.first;
    for (var i in listFromStream) {
      //exclude rides created by current user
      //exclude past rides
      if (i['driver']['phone'] != currentUser &&
          i['dateTime'] >= searchModel.searchDate.millisecondsSinceEpoch) {
  ...
  Future<void> updateRideToConfirmed(UserRide ride) async {
    ...
    for (var i in remoteDoc.docs) {
      try {
        i.reference.update({'status': 'Status.confirmed'});
    ...
    var query2 = userRideCollection
        ...
        .where('ride.fromText', isEqualTo: ride.ride.fromText)
        .where('ride.toText', isEqualTo: ride.ride.toText);
    var remoteDoc2 = await query2.get();
    for (var i in remoteDoc2.docs) {
      try {
        i.reference.update({'status': 'Status.confirmed'});
      } on Exception catch (e) {
        print('couldnt change from pending to confirmed');
  ...
  Future<void> updateRideToCompleted(UserRide ride) async {
    ...
    for (var i in remoteDoc.docs) {
      try {
        i.reference.update({'status': 'Status.completed'});
      } on Exception catch (e) {
  ...
  Future<void> updateRideToFinished(UserRide ride, UserModel reviewee) async {
    ...
    for (var i in remoteDoc.docs) {
      try {
        i.reference.update({'isFinished': true});
      } on Exception catch (e) {
  ...
  Future<List<Map>> getAllUserModelsFromDb() async {
    var remoteDocuments = await db.collection('UserModel').get();
    List<Map> resultDB = [];
    for (var i in remoteDocuments.docs) {
      resultDB.add(i.data());
    }
  ...
  Future<Map> showCurrentUserModel() async {
    ...
    if (result == null) {
      return {'error': ' no meesenges found from that User'};
    } else {
      return result;
    }
  }
