...
class _RidesScreenState extends State<RidesScreen> {
  ...
  void userInfoAlertDIalog(BuildContext context, UserRide userRide,
      String action, UserModel fellowTraveller, List<ReviewModel> reviews) {
    showDialog<void>(
      context: context,
      barrierDismissible: false, // user must tap button!
      builder: (BuildContext context) {
    ...
    return Card(
      ...
      child: ListTile(
        leading: InkWell(
          onTap: () async {
            var userReviews =
                await widget.db.getUserReviewsFromPhone(fellowTraveller.phone);
            //todo show info AND phone/email
            userInfoAlertDIalog(
                context, userRide, 'confirm', fellowTraveller, userReviews);
          },
          child: CircleAvatar(
