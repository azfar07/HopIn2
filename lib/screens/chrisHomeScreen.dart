...
class _HomeScreenState extends State<ChrisHomeScreen> {
  ...
  RidesModel detailsRide = new RidesModel(
      fromText: 'from',
      toText: 'to',
        ...
        child: Padding(
            ...
            child: SearchMapPlaceWidget(
              ...
              onSelected: (Place place) async {
                ...
                _mapController
                    .animateCamera(CameraUpdate.newLatLngZoom(placeCords, 15));
                _clearPolylines();
  ...
  Widget _createScreen(bool isVisible) {
    return Visibility(
        ...
        child: Stack(
          children: <Widget>[
            _topText(),
            _customButton(Alignment.bottomRight, 15.0, Icons.check_circle,
                "Finish", _onFinishPressed),
            _customButton(Alignment.bottomLeft, 15.0, Icons.cancel, "Cancel",
                _onCancelPressed)
          ],
        ));
  }
  ...
  Widget _resultsScreen(bool isVisible, String name, List<RidesModel> results) {
    return Visibility(
      ...
      child: Stack(children: <Widget>[
        _customButton(Alignment.topLeft, 0.0, Icons.arrow_back_ios, "Back",
            _onBackPressed),
        Align(
  ...
  List<Widget> _listResultItem(List<RidesModel> results) {
    ...
    for (final ride in results) {
      items.add(box);
      resultCard =
          new RideResultCard(ridesModel: ride, onPressed: _showRideDetails);
  ...
  Widget createCard(RideResultCard rideResultCard) {
    return GestureDetector(
      onTap: () {
        if (!(polylines
            .containsKey(PolylineId(rideResultCard.ridesModel.driver.name)))) {
          _clearPolylines();
          ...
          _addMarker(
              rideResultCard.ridesModel.randPoints[0],
              "Start",
              BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueBlue),
              "Start");
          _addMarker(
              rideResultCard.ridesModel.toLatLng,
              "End",
  ...
  Widget _detailsScreen(
      bool isVisible, RidesModel ride, List<ReviewModel> list) {
                ...
                child: DecoratedBox(
                  ...
                  child: ListTile(
                    leading: CircleAvatar(
                      backgroundImage: new NetworkImage(driver
                          .getUrlFromNameHash(genderInput: driver.gender)),
                      radius: 28.0,
                    ),
                    title: Text(
                      driver.name,
  ...
  Widget _pickUpPointScreen(bool isVisible) {
    return Visibility(
        visible: isVisible,
        child: _customButton(Alignment.topLeft, 1.0, Icons.arrow_back_ios,
            "Return", _onReturnPressed));
  }
  ...
  _selectPickUpPoint(RidesModel ride) async {
    ...
    _mapController
        .animateCamera(CameraUpdate.newLatLngZoom(ride.randPoints[0], 15));
    if (markers.containsKey(MarkerId("Start")))
      markers.remove(MarkerId("Start"));
    int i = 0;
    for (final point in ride.randPoints) {
      if (i % 3 == 0) _showRendezvousPoint("address$i", point, true);
      i++;
    }
  }
